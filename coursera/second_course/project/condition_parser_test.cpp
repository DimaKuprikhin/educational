...
void TestParseCondition() {
  ...
  }
  {
    ...
    shared_ptr<Node> root = ParseCondition(is);
    // LogicalOperationNode* rootNode = dynamic_cast<LogicalOperationNode*>(root.get());
    // DateComparisonNode* left = dynamic_cast<DateComparisonNode*>(rootNode->left.get());
    // DateComparisonNode* right = dynamic_cast<DateComparisonNode*>(rootNode->right.get());
    // std::cout << (rootNode->op == LogicalOperation::And ? "AND" : "OR") << "\n";
    // // std::cout << rootNode->left->Evaluate(
    // std::cout << "asdf: " << root->Evaluate({2017, 1, 1}, "") << "\n";
    // std::cout << "asdf: " << (Date(2017, 1, 1) >= Date(2017, 1, 1)) << "\n";
    // std::cout << "asdf: " << (Date(2017, 1, 1) < Date(2017, 7, 1)) << "\n";
