...
string CreateRandomString(const size_t size) {
...
void TestTime() {
  const size_t documentsCount = 5000; // 50000
  const size_t wordsInDocument = 1000;
  const size_t uniqueWords = 15000;
  const size_t wordLength = 10; // 100
  const size_t queriesCount = 5000; // 500 000
  const size_t wordsInQuery = 10;

  vector<string> wordsPool;
  wordsPool.reserve(uniqueWords);
  for(size_t i = 0; i < uniqueWords; ++i) {
    wordsPool.push_back(CreateRandomString(wordLength));
  }
  std::cout << "words generated\n";

  string documents;
  documents.reserve(documentsCount * (wordsInDocument * (wordLength + 1) + 1));
  for(size_t i = 0; i < documentsCount; ++i) {
    for(size_t j = 0; j < wordsInDocument; ++j) {
      documents += wordsPool[rand() % uniqueWords];
      documents += " ";
    }
    documents += "\n";
  }
  std::cout << "documents generated\n";

  string queries;
  queries.reserve(queriesCount * (wordsInQuery * (wordLength + 1) + 1));
  for(size_t i = 0; i < queriesCount; ++i) {
    for(size_t j = 0; j < wordsInQuery; ++j) {
      queries += wordsPool[rand() % uniqueWords];
      queries += " ";
    }
    queries += "\n";
  }
  std::cout << "queries generated\n";

  istringstream documentsStream(documents);
  istringstream queriesStream(queries);

  SearchServer server;
  { LOG_DURATION("index");
    server.UpdateDocumentBase(documentsStream);
  }
  ostringstream output;
  { LOG_DURATION("queries");
    server.AddQueriesStream(queriesStream, output);
  }
}
