...
public:
  ...
  AirportCounter(TIterator begin, TIterator end) {
      ...
      for(auto it = begin; it != end; ++it) {
          ++counts[static_cast<size_t>(*it)].second;
      }
  }

  size_t Get(TAirport airport) const {
      return counts[static_cast<size_t>(airport)].second;
  }

  void Insert(TAirport airport) {
      ++counts[static_cast<size_t>(airport)].second;
  }
  ...
  void EraseAll(TAirport airport) {
      counts[static_cast<size_t>(airport)].second = 0;
  }
...
private:
  void Fill() {
      for(size_t i = 0; i < enumSize; ++i) {
          counts[i] = { (TAirport)i, 0 };
      }
  }

  const size_t enumSize = static_cast<size_t>(TAirport::Last_);
  Items counts;
};
